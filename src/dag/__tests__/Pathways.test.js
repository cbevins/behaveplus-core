...
test('Pathways should detect two paths', () => {
  ...
  expect(path1.providers).toContain(
  ...
  expect(
    keyMap.has('spotting.torchingTrees.firebrand.criticalCoverHeight')
  ).toEqual(true)
  const path2 = keyMap.get(
    'spotting.torchingTrees.firebrand.criticalCoverHeight'
  )
  expect(path2.selected.length).toEqual(1)
  expect(path2.selected).toContain(
    'spotting.torchingTrees.firebrand.criticalCoverHeight'
  )
  ...
  expect(path2.providers).toContain(
    'spotting.torchingTrees.firebrand.criticalCoverHeight'
  )
})
