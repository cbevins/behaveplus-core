...
test('1: Casewise test', () => {
  ...
  expect(ros.value.run.length).toEqual(2 * 3 * 5)
  for (let fm = 0, idx = 0; fm < 2; fm += 1) {
    for (let dm = 0; dm < 3; dm += 1) {
      for (let ws = 0; ws < 5; ws += 1, idx += 1) {
        expect(ros.value.run[idx]).sig(
          results[fm][ws][dm],
          5,
          `fm=${fm}, ws=${ws}, dm=${dm}, idx=${idx}`
        )
      }
    }
  }
  ...
  dag.setModeCasewise()
  dag.runInputs(Inputs)
  expect(ros.value.run.length).toEqual(5) //  fm  dm  ws
  expect(ros.value.run[0]).sig(results[0][0][0], 5) //  10   5   0
  expect(ros.value.run[1]).sig(results[1][1][1], 5) // 124  10   5
  expect(ros.value.run[2]).sig(results[1][2][2], 5) // 124  15  10
  expect(ros.value.run[3]).sig(results[1][3][2], 5) // 124  15  15
  expect(ros.value.run[4]).sig(results[1][4][2], 5) // 124  15  20
})
