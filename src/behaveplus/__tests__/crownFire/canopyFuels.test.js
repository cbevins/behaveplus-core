...
test('1: Crown Fire Canopy Fuels (FM 10) benchmarks', () => {
  ...
  if (dag.get('configure.wind.direction').value.current === 'upslope') {
    expect(requiredInputs.length).toEqual(10)
  } else if (
    dag.get('configure.wind.direction').value.current === 'headingFromUpslope'
  ) {
    ...
    dag.get('configure.wind.direction').value.current === 'sourceFromNorth'
  ) {
