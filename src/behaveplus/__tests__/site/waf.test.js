...
const primaryFuelAtMidflame = dag.get(
  'surface.primary.fuel.fire.wind.speed.atMidflame'
)
const secondaryFuelAtMidflame = dag.get(
...
test('1: Select open-canopy WAF only', () => {
  ...
  expect(cfgPrimary.value.current).toEqual('catalog')
  ...
  expect(depth.value.current).toEqual(1)
  // let waf010 = 1.83 / Math.log((20.0 + 0.36 * depth.value.current) / (0.13 * depth.value.current))
  // console.log(waf010) // === 0.36210426360602416
  expect(openWaf.value.current).toEqual(0.36210426360602416)

  // Fuel Model 124 open-canopy WAF
  dag.runInputs([[catalogKey, ['124']]])
  expect(depth.value.current).toEqual(2.1)
  // let waf124 = 1.83/Math.log((20.0+0.36*depth)/(0.13*depth))
  // console.log(waf124) // === 0.4225236169597915
  expect(openWaf.value.current).toEqual(0.4225236169597915)
})
...
test('2: Fuel bed WAF with input vs estimated WAF', () => {
  ...
  expect(primaryFuelWaf.value.run[0]).toEqual(0)
  expect(primaryFuelWaf.value.run[1]).toEqual(0.5)
  expect(primaryFuelWaf.value.run[2]).toEqual(1)
  ...
  expect(depth.value.current).sig(1, 12)
  expect(canopyCover.value.current).sig(0.5, 12)
  expect(crownBase.value.current).sig(10, 12)
  expect(crownHeight.value.current).sig(40, 12)
  expect(crownLength.value.current).sig(30, 12)
  expect(crownRatio.value.current).sig(0.75, 12)
  // let fill = 0.75 * 0.5 / 3  // 0.125
  expect(crownFill.value.current).sig(0.125, 12)
  // let w = BpxLibCanopy.waf(canopyCover.value.current, canopy.crownHeight.value.current,
  //   canopy.crownFill.value.current)
  expect(shelteredWaf.value.current).sig(0.1313664741590494, 12)
  expect(sheltersFuel.value.current).toEqual(true)
  expect(primaryFuelWaf.value.current).sig(0.1313664741590494, 12)
  expect(openWaf.value.current).sig(0.36210426360602416, 12)
  // expect(at20ft.value.current).sig((10 / 0.1313664741590494), 12)

  // If fuel is unsheltered, WAF should be openWaf
  dag.runInputs([[canopyCover, [0]]])
  expect(canopyCover.value.current).sig(0, 12)
  expect(shelteredWaf.value.current).sig(1, 12)
  expect(sheltersFuel.value.current).toEqual(false)
  expect(openWaf.value.current).sig(0.36210426360602416, 12)
  expect(primaryFuelWaf.value.current).sig(0.36210426360602416, 12)
  // expect(at20ft.value.current).sig((10 / 0.36210426360602416), 12)
  ...
  expect(openWaf.value.current).sig(0.4179825632019431, 12)
  expect(primaryFuelWaf.value.current).sig(0.1313664741590494, 12)
  // expect(at20ft.value.current).sig((10/0.1313664741590494), 12)
})
...
test('3: Fuel bed midflame wind speed', () => {
  ...
  expect(primaryFuelAtMidflame.value.current).toEqual(10)
  ...
  expect(primaryFuelAtMidflame.value.current).toEqual(10 / 1.13)
  ...
  expect(primaryFuelAtMidflame.value.current).toEqual(12.3)
  ...
  expect(primaryFuelWaf.value.current).sig(0.1313664741590494, 12)
  expect(openWaf.value.current).toEqual(0.36210426360602416)
  expect(primaryFuelAtMidflame.value.current).sig(10 * 0.1313664741590494, 12)
})
