...
test('3 Combined requires 1041 of 1208 Nodes with 22 selected and 32 input Nodes', () => {
  dag.clearSelected()
  dag.runSelected([
...
function displayUnrequiredNodes (dag) {
  let idx = 0
  let req = 0
  let str = 'const unrequiredNodes = [\n'
  dag.node.map.forEach(node => {
    if (!node.status.isRequired) {
      str += `  '${node.node.key}', // ${idx++}\n`
    } else {
      req++
    }
  })
  str += `]\n//There were ${req} required and ${idx} unused of ${dag.node.map.size}\n`
  console.log(str)
  // fs.writeFile('unrequiredNodes.js', str, function (err) {
  //   if (err) throw err;
  //   console.log('Saved!')
  // })
}
