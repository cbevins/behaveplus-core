...
import { Bpx, BpxConfigPalette, DagJest, SurfaceFire } from '../../../index.js'
...
test('1: Head ros', () => {
  ...
  expect(observedRos.value.current).toEqual(10)
  expect(headRos.value.current).toEqual(10)
  ...
  expect(headRos.method.parms[0]).toEqual(
    dag.get('surface.weighted.fire.spreadRate')
  )

  // Back to standAlone
  dag.runConfigs([['configure.module', 'fireEllipse']])
  // head.spreadRate should be bound to site.fire.observed.spreadRate
  inputNodes = dag.requiredInputNodes()
  expect(inputNodes.length).toEqual(1)
  expect(inputNodes).toContain(observedRos)
...
test('2.1: input fire.observed.firelineIntensity, select head.firelineIntensity', () => {
  ...
  // Results
  expect(observedFli.value.current).toEqual(5000)
  expect(headFli.value.current).toEqual(observedFli.value.current)
  expect(headFli.value.current).toEqual(5000)
})
...
test('2.2: input fire.observed.firelineIntensity, select axis.flameLength', () => {
  ...
  // Results
  expect(observedFli.value.current).toEqual(5000)
  expect(observedFl.value.current).toEqual(SurfaceFire.flameLength(5000))
  expect(headFl.value.current).toEqual(observedFl.value.current)
  expect(headFl.value.current).toEqual(SurfaceFire.flameLength(5000))
})
...
test('2.3: input fire.observed.firelineIntensity, select both head.fli and head.fl', () => {
  ...
  // Results
  expect(observedFli.value.current).toEqual(4000)
  expect(headFli.value.current).toEqual(observedFli.value.current)
  expect(observedFl.value.current).toEqual(SurfaceFire.flameLength(4000))
  expect(headFli.value.current).toEqual(4000)
  expect(headFl.value.current).toEqual(SurfaceFire.flameLength(4000))
})
...
test('2.4: input fire.observed.flameLength, select head.firelineIntensity', () => {
  ...
  // Results
  expect(observedFl.value.current).toEqual(10)
  expect(observedFli.value.current).toEqual(
    SurfaceFire.firelineIntensityFromFlameLength(10)
  )
  expect(headFli.value.current).toEqual(observedFli.value.current)
  expect(headFli.value.current).toEqual(
    SurfaceFire.firelineIntensityFromFlameLength(10)
  )
})
...
test('2.5: input fire.observed.flameLength, select head.flameLength', () => {
  ...
  // Results
  expect(observedFl.value.current).toEqual(20)
  expect(headFl.value.current).toEqual(observedFl.value.current)
  expect(headFl.value.current).toEqual(20)
})
...
test('2.6: input fire.observed.flameLength, select both head.fli and head.fl', () => {
  ...
  // Results
  expect(observedFl.value.current).toEqual(30)
  expect(headFl.value.current).toEqual(observedFl.value.current)
  expect(headFl.value.current).toEqual(30)
  expect(observedFli.value.current).toEqual(
...
test('3.1: input fire.observed.lengthToWidthRatio, select axis.lengthToWidthRatio', () => {
  ...
  // Results
  expect(observedLwr.value.current).toEqual(3)
  expect(axisLwr.value.current).toEqual(observedLwr.value.current)
  expect(axisLwr.value.current).toEqual(3)
})
...
test('3.2: input fire.observed.lengthToWidthRatio, select axis.effectiveWindSpeed', () => {
  ...
  // Results
  expect(observedEws.value.current).toEqual(88 * 4 * (3 - 1))
  expect(axisEws.value.current).toEqual(observedEws.value.current)
  expect(axisEws.value.current).toEqual(88 * 4 * (3 - 1))
})
...
test('3.4: input fire.observed.effectiveWindSpeed, select axis.lengthToWidthRatio', () => {
  ...
  // Results
  expect(observedEws.value.current).toEqual(880)
  expect(observedLwr.value.current).toEqual(3.5)
  expect(axisLwr.value.current).toEqual(observedLwr.value.current)
  expect(axisLwr.value.current).toEqual(3.5)
})
...
test('3.5: input fire.observed.effectiveWindSpeed, select axis.effectiveWindSpeed', () => {
  ...
  expect(observedEws.value.current).toEqual(880)
  expect(axisEws.value.current).toEqual(observedEws.value.current)
  expect(axisEws.value.current).toEqual(880)
})
