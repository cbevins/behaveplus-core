 ...
 * @version 0.1.0
 */
import { Bpx, DagJest, TreeMortality } from '../../../index.js'
...
test('1: Tree mortality stand-alone', () => {
  dag.clearSelected()
  dag.runConfigs([
    ['configure.module', 'treeMortality'],
  ...
  expect(requiredNodes).toContain(scorchHt) // 'mortality.scorchHeight'
  ...
  expect(treeBark.value.current).sig(1.116, 5)
  expect(mortality.value.current).sig(0.872273, 5)
  expect(volScorched.value.current).sig(0.88888888888, 9)
  expect(lenScorched.value.current).sig(40 / 60, 6)
})
...
test('2a: Tree mortality linked to scorchHeight with firelineIntensity input', () => {
  ...
  expect(requiredNodes.length).toEqual(12)
  ...
  // and 1 intermediate Nodes:
  expect(requiredNodes).toContain(scorchHt) // 'surface.fire.ellipse.head.scorchHeight'
})
...
test('2b: Tree mortality linked to scorchHeight with flameLength input', () => {
  ...
  expect(requiredNodes).toContain(
    dag.get('site.fire.observed.firelineIntensity')
  )
})

test('3: Tree mortality linked to fireEllipse', () => {
  dag.clearSelected()
  dag.runConfigs([
  ...
  expect(requiredNodes).toContain(
    dag.get('surface.fire.ellipse.head.firelineIntensity')
  )
  expect(requiredNodes).toContain(
    dag.get('surface.fire.ellipse.wind.speed.atMidflame')
  )
  expect(requiredNodes).toContain(
    dag.get('surface.fire.ellipse.head.scorchHeight')
  )
  expect(requiredNodes).toContain(scorchHt) // 'mortality.scorchHeight'
})

test('4: Tree mortality linked to surface fire', () => {
  dag.clearSelected()
  dag.runConfigs([
    ['configure.module', 'surfaceFire'],
    ['configure.fire.firelineIntensity', 'firelineIntensity']
...
test('5: Tree mortality results validation with BP6', () => {
  dag.clearSelected()
  dag.runConfigs([['configure.module', 'treeMortality']])
  dag.runSelected([
    [mortality, true],
    ['site.canopy.tree.barkThickness', true],
    ['mortality.crownLengthScorched', true],
  ...
  for (let idx = 0; idx < species.length; idx++) {
    ...
    expect(treeSpecies.value.run[idx]).toEqual(spp)
    expect(volScorched.value.run[idx]).sig(0.88888888888, 9)
    expect(lenScorched.value.run[idx]).sig(40 / 60, 6)
    if (Results.hasOwnProperty(spp)) {
      expect(mortality.value.run[idx]).sig(Results[spp][0], 5, `${spp}`)
